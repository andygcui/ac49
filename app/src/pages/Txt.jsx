import { Link } from 'react-router-dom'

function Txt() {
  return (
    <div className="min-h-screen bg-white">
      <div className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          {/* Header */}
          <header className="text-center mb-16">
            <h1 className="text-[2rem] font-serif text-gray-900 mb-4">
              .txt
            </h1>
            
            <p className="text-gray-600 hover:underline hover:text-orange-700">
            <Link to="/txt/aug26" className="block mb-3 cursor-text"> august </Link>
            </p>

            <p className="text-gray-600 hover:underline hover:text-orange-700">
            <Link to="/txt/july26" className="block mb-3 cursor-text"> july </Link>
            </p>

            <p className="text-gray-600 hover:underline hover:text-orange-700">
            <Link to="/txt/june26" className="block mb-3 cursor-text"> june </Link>
            </p>

            <p className="text-gray-600 hover:underline hover:text-orange-700">
            <Link to="/txt/may26" className="block mb-3 cursor-text"> may </Link>
            </p>

            <p className="text-gray-600 hover:underline hover:text-orange-700">
            <Link to="/txt/apr26" className="block mb-3 cursor-text"> april </Link>
            </p>

            <p className="text-gray-600 hover:underline hover:text-orange-700">
            <Link to="/txt/mar26" className="block mb-3 cursor-text"> march </Link>
            </p>

            <p className="text-gray-600 hover:underline hover:text-orange-700">
            <Link to="/txt/feb26" className="block mb-3 cursor-text"> february </Link>
            </p>

            <p className="text-gray-600 hover:underline hover:text-orange-700">
            <Link to="/txt/jan26" className="block mb-3 cursor-text"> january </Link>
            </p>

            <p className="text-gray-600 hover:text-orange-700 hover:underline">
            <Link to="/txt/2025" className="block mb-3 cursor-text"> 2025 notes</Link>
            </p>

            <p className="text-gray-600 hover:text-orange-700 hover:underline">
            <Link to="/txt/dec25" className="block mb-3 cursor-text"> december </Link>
            </p>

          </header>
        </div>
      </div>
    </div>
  )
}

export default Txt
